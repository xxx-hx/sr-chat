...
app.get('/api/chat-list', async (req, res) => {
    ...
    const last_chat_id = req.query.last_chat_id;

    if (!room_id || !last_chat_id) {
        return res.status(400).json({ error: 'room_id dan last_chat_id wajib diisi di query string' });
    }

    try {
        const response = await axios.get(`https://www.showroom-live.com/fan_room/get_talk_list?room_id=${room_id}&last_chat_id=${last_chat_id}`,
...
app.get('/api/room-info', async (req, res) => {
    const room_key = req.query.room_key;

    if (!room_key) {
        return res.status(400).json({ error: 'room_key wajib diisi di query string' });
    }
...
app.post('/api/send-message', async (req, res) => {
    ...
    try {
        ...
        if (response.data && response.data.ok === 1) {
            ...
            return res.status(400).json({ success: false, error: response.data?.error || 'Gagal mengirim pesan' });
        }
    } catch (error) {
        if (error.response) {
            return res.status(error.response.status).json({ success: false, error: error.response.data?.error || error.response.statusText });
        } else if (error.request) {
            return res.status(500).json({ success: false, error: 'Tidak ada response dari server Showroom' });
        } else {
